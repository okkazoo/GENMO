...
$ZipFiles = Get-ChildItem -Path $DownloadsDir -Filter "*.zip" | Where-Object { $_.Name -match "SMPL|smpl" }
...
foreach ($PklFile in $AllPkls) {
    ...
    # Check for neutral model
    if ($Name -match "neutral|NEUTRAL") {
    ...
    # Check for female model
    if ($Name -match "female|_f_") {
    ...
    # Check for male model (but not female)
    if ($Name -match "male|_m_") {
        if ($Name -notmatch "female") {
...
Write-Host "   Set-Item -Path Env:VASTAI_API_KEY -Value 'your_api_key'" -ForegroundColor White
